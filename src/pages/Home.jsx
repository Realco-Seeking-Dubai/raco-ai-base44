  ...
  return (
    <div className="p-6 max-w-5xl mx-auto animate-fade-in">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-8">
        <KpiCard label="Active Network" value={loading ? '…' : '—'} icon={Users} color="evergreen" />
        <KpiCard label="Open Tasks" value={loading ? '…' : tasks.length} icon={CheckCircle2} color="brass" />
        <KpiCard label="AI Suggestions" value={loading ? '…' : suggestions.length} icon={Zap} color="sky" />
        <KpiCard label="Hot Opportunities" value={loading ? '…' : suggestions.filter(s => s.priority_score > 0.8).length} icon={Star} color="terracotta" />
      </div>
        ...
        <div className="bg-card border border-hairline rounded-xl overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-hairline">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium">Raco Suggestions</span>
            </div>
